...
import System.Posix (sigTERM)
import System.Posix.Process (executeFile, getParentProcessID)
...
defaultCliOptions = CliOptions False False
...
recompileIfStale src out = do
    ...
    (code, _, err) <-
        readProcessWithExitCode
            "cabal"
            ["exec", "ghc", "--", "--make", src, "-o", out, "-rtsopts", "-threaded", "-XBlockArguments", "-O2"]
            ""
    case code of
        ExitSuccess -> pure ()
        ExitFailure _ -> do
            hPutStrLn stderr $ "\ESC[1;31m" <> replicate 40 '─' <> "\ESC[0m"
            hPutStrLn stderr "\ESC[1;31mInvalid Rivulet config! Failed to compile:\ESC[0m"
            hPutStrLn stderr $ "\ESC[1;31m" <> err <> "\ESC[0m"
            hPutStrLn stderr $ "\ESC[1;31m" <> replicate 40 '─' <> "\ESC[0m"
